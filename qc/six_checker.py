...
def populate_six_using_temps_from_qc_table(start_date, end_date, source_id, stations):
    ...
    for station in stations:
        tmin = get_temps_for_year_from_qc_table(station['station_id'], 'tmin', source_id, year)
        tmax = get_temps_for_year_from_qc_table(station['station_id'], 'tmax', source_id, year)
        if tmin is not None and tmax is not None:
            tmin = np.array(tmin[:num_days])
            tmax = np.array(tmax[:num_days])
            populate_six_in_qc_table(tmax, tmin, station['latitude'], station['station_id'], source_id, year)
