...
filename = f'/geo-data/gridded_models/japanese_beetle/japanese_beetle_adult/japanese_beetle_adult_{today.strftime("%Y%m%d")}.tif'
with open(filename, 'wb') as out_file:
   content = requests.get(japanese_beetle_adult_url, stream=True).content
   out_file.write(content)
   time_series_table = 'japanese_beetle_adult'
   tif_name = f'japanese_beetle_adult_{today.strftime("%Y%m%d")}.tif'
   if table_exists(time_series_table):
      update_time_series(time_series_table, tif_name, today)
set_srs(filename)

filename = f'/geo-data/gridded_models/japanese_beetle/japanese_beetle_egg_hatch/japanese_beetle_egg_hatch_{today.strftime("%Y%m%d")}.tif'
with open(filename, 'wb') as out_file:
   content = requests.get(japanese_beetle_egg_url, stream=True).content
   out_file.write(content)
   time_series_table = 'japanese_beetle_egg_hatch'
   tif_name = f'japanese_beetle_egg_hatch_{today.strftime("%Y%m%d")}.tif'
   if table_exists(time_series_table):
      update_time_series(time_series_table, tif_name, today)
set_srs(filename)
