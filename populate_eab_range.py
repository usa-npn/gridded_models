...
def process_eab_data(start_date, end_date):
    """
    Process eab data for a date range.
    ...
    while current_date <= end_date:
        ...
        filename = f'/geo-data/gridded_models/eab/eab_adult/eab_adult_{current_date.strftime("%Y%m%d")}.tif'
        try:
            with open(filename, 'wb') as out_file:
                content = requests.get(eab_adult_url, stream=True).content
                out_file.write(content)
                time_series_table = 'eab_adult'
                tif_name = f'eab_adult_{current_date.strftime("%Y%m%d")}.tif'
                if table_exists(time_series_table):
                    update_time_series(time_series_table, tif_name, current_date)
            set_srs(filename)
        except Exception as e:
            print(f"Error processing adult eab data for {current_date}: {e}")
        
        # Process egg hatch data
        filename = f'/geo-data/gridded_models/eab/eab_egg_hatch/eab_egg_hatch_{current_date.strftime("%Y%m%d")}.tif'
        try:
            with open(filename, 'wb') as out_file:
                content = requests.get(eab_egg_url, stream=True).content
                out_file.write(content)
                time_series_table = 'eab_egg_hatch'
                tif_name = f'eab_egg_hatch_{current_date.strftime("%Y%m%d")}.tif'
                if table_exists(time_series_table):
                    update_time_series(time_series_table, tif_name, current_date)
            set_srs(filename)
        except Exception as e:
...
if __name__ == "__main__":
    ...
    start_date = date(2025, 6, 22)  # Change to your desired start date
    ...
    process_eab_data(start_date, end_date)
