...
def process_slf_data(start_date, end_date):
    """
    Process slf data for a date range.
    ...
    while current_date <= end_date:
        ...
        filename = f'/geo-data/gridded_models/slf/slf_adult/slf_adult_{current_date.strftime("%Y%m%d")}.tif'
        try:
            with open(filename, 'wb') as out_file:
                content = requests.get(slf_adult_url, stream=True).content
                out_file.write(content)
                time_series_table = 'slf_adult'
                tif_name = f'slf_adult_{current_date.strftime("%Y%m%d")}.tif'
                if table_exists(time_series_table):
                    update_time_series(time_series_table, tif_name, current_date)
            set_srs(filename)
        except Exception as e:
            print(f"Error processing adult slf data for {current_date}: {e}")
        
        # Process egg hatch data
        filename = f'/geo-data/gridded_models/slf/slf_egg_hatch/slf_egg_hatch_{current_date.strftime("%Y%m%d")}.tif'
        try:
            with open(filename, 'wb') as out_file:
                content = requests.get(slf_egg_url, stream=True).content
                out_file.write(content)
                time_series_table = 'slf_egg_hatch'
                tif_name = f'slf_egg_hatch_{current_date.strftime("%Y%m%d")}.tif'
                if table_exists(time_series_table):
                    update_time_series(time_series_table, tif_name, current_date)
            set_srs(filename)
        except Exception as e:
            print(f"Error processing slf egg hatch data for {current_date}: {e}")
...
if __name__ == "__main__":
    ...
    end_date = date(2025, 7, 21)  # Change to your desired end date
    ...
    process_slf_data(start_date, end_date)
